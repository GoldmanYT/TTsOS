...
## Выводы
1. Параметр $n$ отвечает за количество отсчётов и размер окна.  
   При увеличении параметра $n$ размер первого лепестка уменьшается.  
   При чётном значении $n$ вершина графика плоская, при нечётном - острая.
3. Параметр $Rs$ отвечает за "высоту", до которой будут "спускаться" лепестки.  
   При увеличении параметра $Rs$ размер первого лепестка увеличивается.
